...
pub fn create_full<C, P>(
	...
	where
		...
		C::Api: runtime_api::NodeVotingRuntimeApi<Block, AccountId>,
		...
		P: TransactionPool + 'static
{
	...
	use rpc_api::voting::{ Voting, VotingApiServer };
	let mut io: RpcModule<()> = RpcModule::new(());
	let FullDeps { client, pool, deny_unsafe } = deps;

	io.merge(System::new(client.clone(), pool.clone(), deny_unsafe).into_rpc())?;
	io.merge(TransactionPayment::new(client.clone()).into_rpc())?;
	io.merge(Referral::new(client.clone()).into_rpc())?;
	io.merge(Voting::new(client).into_rpc())?;
